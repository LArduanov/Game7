...
public class Dragon extends Flying{
    public Dragon(String name, int hp, int attack, int armor, int stamina, int money, Inventory weapons,Inventory armors){
        this.name = name;
        this.hp = hp;
        this.attack = attack;
        this.armor = armor;
    ...
    public void attack (Flying target){
        ...
        if (hit > 0) {
            if (target.hp > 0) {
                target.hp -= hit;
                System.out.println(name + " дышит огнем на " + target.name + "а мощностью " + hit);
                weaponStrength();
                if (target.hp > 0) {
    ...
    public void attack (Humanoid target) {
        ...
        if (hit > 0) {
            if (target.hp > 0) {
                target.hp -= hit;
                System.out.println(name + " бьет хвостом " + target.name + "а мощностью " + hit);
                weaponStrength();
                if (target.hp > 0) {
                    System.out.println("Здоровье " + target.name + ": " + target.hp);
                    target.armorStrength();
